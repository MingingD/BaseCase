...
class RuleBasedLegalClassifier:
    def __init__(self, category_keywords, low_confidence_threshold = 0.02, ambiguity_margin = 0.03):
        """
        category_keywords:
            dictionary that has the keywords for each category

        low_confidence_threshold:
            if best normalized score is below this, ask user to manually choose

        ambiguity_margin:
            if topX - topY is below this, ask user to choose between top X and Y
        """
    ...
    def compute_scores(self, query):
        ...
        for category, keyword_weights in self.category_keywords.items():
            ...
            for keyword, weight in keyword_weights.items():
                if self.keyword_in_text(keyword, processed_query):
                    raw_score += weight
